...
<body>
    ...
    if(isset($_POST['Signup'])){  
	   ...
	   if(result -> mysqli_query($con, $sql) > 0){
			...
			if(!empty($email)){
				$err_email = "";
			}else $err_email = "Vui lòng nhập email";
	
			if(!empty($pasword)){
				$err_pasword = "";
			}else $err_pasword = "Vui lòng nhập pass";
	
			if((!empty($name)) && (!empty($email)) && (!empty($pasword))){
				$sql = "INSERT INTO `user`(`name`, `email`, `password`) VALUES ('".$name."', '".$email."', '".md5($pasword)."');";
				echo $sql;
